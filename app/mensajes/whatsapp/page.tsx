      ...
      <Head>
        ...
        <div className='p-4 lg:p-6 w-full h-full flex gap-6 flex-col md:flex-row overflow-y-auto bg-bg dark:bg-neutral-900'>
          <div className='flex flex-col gap-6 w-full overflow-y-auto lg:w-1/2'>
            ...
            <div className='w-full flex flex-col gap-2 max-h-[100vh] overflow-y-auto'>
              {
                phones === undefined
                  ? (
          ...
          <div className='w-full lg:w-1/2'>
            <div className='bg-white flex flex-col justify-between border border-black/5 rounded-xl w-full h-[60vh] md:h-full dark:bg-neutral-800 dark:border-neutral-700' style={{ boxShadow: '0px 3px 10px 3px #11111108' }}>
                ...
                selectedPhone
                  ? (
                    <div className='flex gap-4 border-b px-4 py-6'>
                      <p className='my-auto'>{selectedPhone}</p>
                      <select onChange={async (e: any) => {
                        await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/whatsapp-tag/${selectedPhone}`, { tag: e.target.value })
                        getMessages()
                      }} className='px-2 py-1 rounded-lg' style={{ backgroundColor: chatTags?.find((chatTag: any) => chatTag.tag === phones?.find(phone => selectedPhone === phone.phone)?.tag)?.color, color: '#ffffff' }} value={chatTags?.find((chatTag: any) => chatTag.tag === phones?.find(phone => selectedPhone === phone.phone)?.tag)?.tag}>
                        {
              ...
              <div ref={containerRef} className='w-full p-4 h-full pr-4 flex flex-col' style={{ overflow: 'overlay' }}>
                {
              ...
              <form onSubmit={async (e: any) => {
                ...
                setNewMessage('')
                await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/whatsapp`, {phone: selectedPhone, response: newMe, agent: true, view: false})
                getMessages()
