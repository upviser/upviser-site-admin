...
export const PopupDeleteFunnel: React.FC<Props> = ({ popupDeleteFunnel, setPopupDeleteFunnel, selectFunnel, setFunnels, getFunnels }) => {
  ...
  return (
      ...
      }} className={`${popupDeleteFunnel.view} ${popupDeleteFunnel.opacity} transition-opacity duration-200 fixed w-full h-full bg-black/30 flex top-0 left-0 z-50`}>
        <div onMouseEnter={() => setPopupDeleteFunnel({ ...popupDeleteFunnel, mouse: true })} onMouseLeave={() => setPopupDeleteFunnel({ ...popupDeleteFunnel, mouse: false })} className={`${popupDeleteFunnel.opacity === 'opacity-1' ? 'scale-1': 'scale-90'} transition-transform duration-200 w-full max-w-[500px] max-h-[600px] overflow-y-auto p-5 rounded-xl flex flex-col gap-4 m-auto border bg-white shadow-popup dark:shadow-popup-dark dark:bg-neutral-800 dark:border-neutral-700`}>
